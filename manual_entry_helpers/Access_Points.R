...
AP_General <- sp_provider_service_geography_served_by_county %>%
  select(-provider_service_code_id) %>%
...
access_point_manual <- AP_General %>%
  ...
  left_join(sp_provider %>%
              select(provider_id, name), by = "provider_id") %>%
  left_join(id_cross %>%
              select(Legacy_ProgramID, Clarity_AgencyName, Clarity_ProgramName), 
            by = c("provider_id" = "Legacy_ProgramID")) %>%
  relocate(c(name:Clarity_ProgramName), .before = provider_id) %>%
  rename("SP_ProjectName" = name, "SP_ProjectID" = provider_id)
